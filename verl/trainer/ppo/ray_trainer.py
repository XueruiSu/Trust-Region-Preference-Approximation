...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns, id_tensor = core_algos.compute_trpa_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
        data.batch['id_tensor'] = id_tensor
...
def compute_reward_metrics(batch):
    ...
    # Calculate missing answer ratio (value == -1)
    format_error = torch.sum(reward_tensor == -3).float() / reward_tensor.numel()
    reward_metrics["reward/wrong_format"] = format_error.detach().item()
...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.ActorRollout) # 这个地方将actor model加载到虚拟环境池中，Role.ActorRollout会在主函数（main_ppo）中声明和哪个类对应，关键词：role_worker_mapping
        ...
        elif self.config.algorithm.adv_estimator == 'trpa':
            self.use_critic = False
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if not self.config.algorithm.preference_reward:
                            reward_tensor = self.reward_fn(batch)
                        else:
                            reward_tensor, preference_levels = self.reward_fn(batch, preference_reward=self.config.algorithm.preference_reward)
                            batch.batch['preference_levels'] = preference_levels
